...
from evaluation.cross_frequency_evaluator import (
    run_cfg_selection,
    run_test_evaluation,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate the locked 8x8 4.9GHz-to-6.7GHz RadioFlow experiment"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    select = subparsers.add_parser(
        "select-cfg",
        help="select CFG on the frozen 4.9GHz validation split",
    )
