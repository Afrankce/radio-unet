...
CODE_ROOT="${RADIOFLOW_ATTENTION_FNO_CODE_ROOT:-/home/wys/radioflow_20260823/attention-fno-singlebeam}"
DATASET_ROOT="${RADIOFLOW_ATTENTION_FNO_DATASET_ROOT:-/home/wys/radioflow_20260823/datasets/MultiConfigRadiomap}"
RESULT_ROOT="${RADIOFLOW_ATTENTION_FNO_RESULT_ROOT:-/home/wys/radioflow_20260823/results/attention_fno_samefreq_6.7ghz}"
ENV_FILE="${RADIOFLOW_ATTENTION_FNO_ENV_FILE:-/home/wys/radioflow_20260823/radioflow_remote_env.sh}"
...
build_command() {
  ...
  COMMAND=(
    python -u "$CODE_ROOT/train_same_frequency_attention_fno.py"
...
if [[ ! -d "$CODE_ROOT" ]]; then
  echo "attention-FNO checkout is missing: $CODE_ROOT" >&2
  exit 1
fi
...
if [[ "$MODE" == "--smoke" ]]; then
  ...
  if [[ "$failed" -ne 0 ]]; then
    echo "one or more attention-FNO smoke runs failed; inspect smoke logs" >&2
    exit 1
  fi
