...
: "${RUN_ROOT:?set RUN_ROOT to the parent run directory}"
: "${RESULTS_ROOT:?set RESULTS_ROOT to the parent result directory}"
...
for index in "${!arrays[@]}"; do
  ...
  if [[ "$MODE" == "train" ]]; then
    mkdir -p "$run_dir"
    "$PYTHON_BIN" run_same_frequency_multiscale_uno.py train \
    ...
    (
      "$PYTHON_BIN" run_same_frequency_multiscale_uno.py select-cfg \
        --dataset-root "$DATASET_ROOT" \
        --manifest-path "$manifest" \
        --height-stats-path "$HEIGHT_STATS" \
        --run-root "$run_dir" \
        ...
        --array-size "$array" \
        --device "cuda:$gpu"
      "$PYTHON_BIN" run_same_frequency_multiscale_uno.py test \
        --dataset-root "$DATASET_ROOT" \
        --manifest-path "$manifest" \
        --height-stats-path "$HEIGHT_STATS" \
        --run-root "$run_dir" \
