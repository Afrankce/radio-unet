...
from training.same_frequency_attention_fno_trainer import (
...
def _config(tmp_path: Path) -> AttentionFNOTrainConfig:
...
def _tiny_attention(model_size: str) -> AttentionConditionedFNO2d:
    assert model_size == ATTENTION_FNO_MODEL_SIZE
...
def test_attention_fno_run_config_is_written_and_revalidated(tmp_path: Path) -> None:
    cfg = _config(tmp_path)
    controls = InvocationControls(resume="auto")

    first = write_or_validate_attention_fno_run_config(cfg, controls)
    second = write_or_validate_attention_fno_run_config(cfg, controls)
    record = json.loads(first.read_text(encoding="utf-8"))

    assert first == second == cfg.run_dir / "config.json"
    assert record["model_size"] == ATTENTION_FNO_MODEL_SIZE
    assert record["config_sha256"] == cfg.config_sha256


def test_one_step_attention_fno_smoke_writes_reloadable_full_state_checkpoint(
    tmp_path: Path,
    monkeypatch,
) -> None:
    import training.same_frequency_attention_fno_trainer as module
    ...
    monkeypatch.setattr(module, "build_same_frequency_backbone", _tiny_attention)
    cfg = _config(tmp_path)

    result = run_same_frequency_attention_fno_training(
        cfg,
    ...
    assert payload["run_identity"]["model_size"] == ATTENTION_FNO_MODEL_SIZE
