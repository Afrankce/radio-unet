...
from train_same_frequency_fno import build_parser


def test_fno_cli_exposes_only_operational_controls() -> None:
...
def test_fno_cli_keeps_stop_and_smoke_mutually_exclusive() -> None:
    parser = build_parser()
    common = [
        "--dataset-root",
        "dataset",
    ...
    try:
        parser.parse_args(
            common
            + ["--stop-after-epoch", "1", "--smoke-optimizer-steps", "1"]
        )
