...
from training.same_frequency_fno_config import PaperFNOTrainConfig
from training.same_frequency_fno_trainer import run_same_frequency_fno_training
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train the locked paper-FNO backbone on the 6.7GHz-to-6.7GHz "
            "zero-degree single-beam RadioFlow protocol"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    beam_id, _ = infer_manifest_selection(
