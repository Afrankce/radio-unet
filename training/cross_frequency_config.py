...
class CrossFrequencyTrainConfigError(ValueError):
    """A cross-frequency run attempts to change a locked experiment control."""


@dataclass(frozen=True)
class CrossFrequencyTrainConfig:
    dataset_root: Path
    manifest_path: Path
    height_stats_path: Path
    run_root: Path
    array_size: Literal["8x8"] = ARRAY_SIZE
    model_size: Literal["lite", "large"] = "lite"
    train_scale: float = 1.0
    train_frequency_hz: int = TRAIN_FREQUENCY_HZ
    val_frequency_hz: int = TRAIN_FREQUENCY_HZ
    ...
    def __post_init__(self) -> None:
        ...
        if self.array_size != ARRAY_SIZE:
            raise CrossFrequencyTrainConfigError(
                f"array_size is locked to {ARRAY_SIZE}, got {self.array_size!r}"
            )
        ...
        if self.train_scale != 1.0 or type(self.train_scale) is not float:
            raise CrossFrequencyTrainConfigError("train_scale is locked to 1.0")
        locked: dict[str, Any] = {
            "train_frequency_hz": TRAIN_FREQUENCY_HZ,
        ...
        if not math.isfinite(self.steering_deg):
            raise CrossFrequencyTrainConfigError("steering_deg must be finite")
    ...
    def scientific_payload(self) -> dict[str, Any]:
        return {
            ...
            "scene_split_claim": "joint_cross_frequency_and_scene_disjoint_generalization",
            "normalization": {
    ...
    @classmethod
    def from_json(cls, text: str) -> "CrossFrequencyTrainConfig":
        try:
        ...
        if not isinstance(payload, dict):
            raise CrossFrequencyTrainConfigError("run config root must be an object")
        try:
