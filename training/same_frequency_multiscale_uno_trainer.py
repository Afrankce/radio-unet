...
def write_or_validate_multiscale_uno_run_config(
    ...
    if path.exists():
        try:
            existing = MultiscaleUNOTrainConfig.from_json(
                path.read_text(encoding="utf-8")
            )
        except (OSError, UnicodeError, ValueError) as error:
            raise MultiscaleUNOConfigError(
                f"invalid existing multiscale-UNO run config {path}: {error}"
            ) from error
        if existing.config_sha256 != cfg.config_sha256:
            raise MultiscaleUNOConfigError(
                "existing multiscale-UNO run config scientific hash mismatch"
            )
        return path
...
def _validate_smoke_checkpoint_fresh(
    ...
    *,
    ...
    model = build_same_frequency_backbone(MULTISCALE_UNO_MODEL_SIZE)
    ...
    if state.optimizer_step <= 0:
        raise SameFrequencyTrainerContractError(
            "fresh multiscale-UNO smoke reload has no optimizer progress"
        )
...
def run_same_frequency_multiscale_uno_training(
    ...
    *,
    ...
    if not isinstance(cfg, MultiscaleUNOTrainConfig):
        raise MultiscaleUNOConfigError("cfg must be a MultiscaleUNOTrainConfig")
    ...
    if (
        ...
        raise SameFrequencyTrainerContractError(
            "resume=none refuses an existing multiscale-UNO last.pt"
        )
    write_or_validate_multiscale_uno_run_config(effective_cfg, controls)
